...
while ( keys %toDo_models ) {
    ...
    foreach my $clus_idx ( sort { $toDo_models{$a} <=> $toDo_models{$b} } keys %toDo_models ) {
        my $model_dir = "$CLUSTER_DIR/$clus_idx.cluster/MODEL";
        ...
        writeSet( $ids_aref, "$CLUSTER_DIR/$clus_idx.cluster/center.ids" );
        writeSubsetFrags( \@fa, $ids_aref, "$CLUSTER_DIR/$clus_idx.cluster/center.frags", "SEQ" );
        writeSubsetFasta( \@fa, $ids_aref, "$CLUSTER_DIR/$clus_idx.cluster/center.fa", 1 );
        my $center_fa_file = "$CLUSTER_DIR/$clus_idx.cluster/center.fa";

        my $ids_all = readSubset( $fast_cluster, 1, $nspdk_knn_center * 3 );

        writeSet( $ids_all, "$CLUSTER_DIR/$clus_idx.cluster/center.ids.ext" );
        writeSubsetFasta( \@fa, $ids_all, "$CLUSTER_DIR/$clus_idx.cluster/center.fa.ext", 1 );
        ...
        my $ids_knn = readSubset( "$CLUSTER_DIR/$clus_idx.cluster/center.ids", 1 );

        open( OUT, ">$CLUSTER_DIR/$clus_idx.cluster/names" );
        map { print OUT $_ . "\n"; } sort { $a <=> $b } @{$ids_knn};
        close(OUT);


        my $tree_dir = "$CLUSTER_DIR/$clus_idx.cluster/TREE";
        system("mkdir -p $tree_dir");

        sim2matrix( "$CLUSTER_DIR/$clus_idx.cluster/center.sim", $ids_knn, "$tree_dir/matrix.kernel" );


        matrix_blacklist_overlap_frags( "$tree_dir/matrix.kernel", "$CLUSTER_DIR/$clus_idx.cluster/center.ids", $map_data, "$tree_dir/matrix.kernel.ol", 0.1, 6 );


        system("cp $tree_dir/matrix.kernel.ol $tree_dir/matrix.tree");

        my $tree_file = "$tree_dir/tree";
        matrix2tree( "$tree_dir/matrix.tree", "$CLUSTER_DIR/$clus_idx.cluster/names", $tree_dir, $tree_file );
...
#~ writeSet( $ids_aref, "$CLUSTER_DIR/center.ids" );

#~ my @fa = read_fasta_file($data_fasta);

#~ writeSubsetFrags( \@fa, $ids_aref, "$CLUSTER_DIR/center.frags", "SEQ" );

#~ writeSubsetFasta( \@fa, $ids_aref, "$CLUSTER_DIR/center.fa", 1 );



#~ my $center_fa_file = "$CLUSTER_DIR/center.fa";
#~ #my $center_fa_file2 = "$CLUSTER_DIR/center2.fa";

#~ my $ids_all = readSubset( $fast_cluster, 1, $nspdk_knn_center * 3 );

#~ writeSet( $ids_all, "$CLUSTER_DIR/center.ids.ext" );
#~ writeSubsetFasta( \@fa, $ids_all, "$CLUSTER_DIR/center.fa.ext", 1 );

#~ my $knn_sim = readSubset( $fast_cluster_sim, 1 );

#~ open( OUT, ">$CLUSTER_DIR/center.sim" );
		#~ print OUT join( " ", @{$knn_sim} ) . "\n";
#~ close(OUT);

#~ my $ids_knn = readSubset( "$CLUSTER_DIR/center.ids", 1 );

#print " ids =  $ids_knn";

#~ open( OUT, ">$CLUSTER_DIR/names" );
#~ map { print OUT $_ . "\n"; } sort { $a <=> $b } @{$ids_knn};
#~ close(OUT);
...
#~ my $tree_dir = "$CLUSTER_DIR/TREE";
#~ system("mkdir -p $tree_dir");

#~ sim2matrix( "$CLUSTER_DIR/center.sim", $ids_knn, "$tree_dir/matrix.kernel" );
#~ ##### stex es data.map haytni chi te vortexica ekel. aysiqn parzel te es uxaki input data a, te inch vor mi tex stexcvuma.
#~ #matrix_blacklist_overlap_frags( "$tree_dir/matrix.kernel", "$CLUSTER_DIR/center.ids", "data.map", "$tree_dir/matrix.kernel.ol", 0.1, 6 );
#~ matrix_blacklist_overlap_frags( "$tree_dir/matrix.kernel", "$CLUSTER_DIR/center.ids", $map_data, "$tree_dir/matrix.kernel.ol", 0.1, 6 );
...
#~ my $tree_file = "$tree_dir/tree";
#~ matrix2tree( "$tree_dir/matrix.tree", "$CLUSTER_DIR/names", $tree_dir, $tree_file );

#~ die "No tree file found ($tree_file)! Exit...\n\n" if ( !-e $tree_file );

### mlocarna step: creates multiple alignment
#~ $mloc_opts = "-p 0.001 --max-diff-am 50 --tau 50  --max-diff 100 --alifold-consensus-dp";

##to do:
### change path to global
#~ system("/home/sokhoyae/Desktop/GalaxyProject/galaxy/tools/fromNspdkToCmfinder/locarna/1.7.16/bin/mlocarna $mloc_opts --treefile $tree_file $center_fa_file --tgtdir $tree_dir/mloc") == 0 or die " mloc command was unable to run to completion:\n\n";
...
#~ system("mkdir -p $model_dir");
 ...
 #~ my $ids_ext = readSubset( "$CLUSTER_DIR/center.ids.ext", 1 );
 #~ my @fa_ext_all = read_fasta_file("$CLUSTER_DIR/center.fa.ext");
 #~ my $fa_ext_merged = mergeFrags( \@fa_ext_all );
 #~ writeSubsetFasta( $fa_ext_merged, $fa_ext_merged->[1], "$model_dir/cmfinder.fa", 1 );


#~ print "echo !!!!!!!!!!!pythonic araj printov!!!!!!!!!!!!!!!";
#~ system("echo !!!!!!!!!!!pythonic araj echoyov!!!!!!!!!!!!!!!");


#~ #my $cmd = "python clustalToSto.py -i $tree_dir/mloc/results/result.aln -o $model_dir/model.tree.stk ";

#~ system ("python /home/sokhoyae/Desktop/GalaxyProject/galaxy/tools/fromNspdkToCmfinder/clustalToSto.py -i $tree_dir/mloc/results/result.aln -o $model_dir/model.tree.stk ") == 0 or die "python command was unable to run to completion:\n\n";

#system("python clustalToSto.py -i $tree_dir/mloc/results/result.aln -o $test_dir/model.tree.stk ");




#~ system("echo !!!!!!!!!!!catic araj!!!!!!!!!!!!!!!");
#~ system("cat $model_dir/model.tree.stk ");
#~ system("echo !!!!!!!!1catic heto!!!!!!!!!!!");

#~ system("rm -f $model_dir/model.cmfinder.stk");
#~ system("python clustalToSto.py -i $tree_dir/mloc/results/result.aln -o $model_dir/model.tree.stk ");
#~ system("cmfinder --g 1.0 -a $model_dir/model.tree.stk $model_dir/cmfinder.fa  $model_dir/model.cmfinder.cm > $model_dir/model.cmfinder.stk");
